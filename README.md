...
- Where is the *findAll* method implemented?
  ...
  The CustomerRepository implements the MongoRepository Interface, 
  which extends PagingAndSortingRepository, CrudRepository and Finally Repository.
  
  The findAll method is present in these interfaces but is not implemented directly,
  this method is implemented in the Spring Repository by SpringFramework and this method was 
  generated using meta-programming.
...
- Suppose you have more than 1000 products in your database. How would you implement a method for supporting pagination and return pages of 50 products to your frontend?
  ...
  usage
        productRepository.findAll(PageRequest.of(page number, 50))
...
- How many products are returned by the *findByDescriptionContaining* query? Why?
  ...
  Two because the query is being paginated in 2-items pages, and we only are getting the first page.
  
- Which are the collection names where the objects are stored? Where are those names assigned?

  the collections names are in the mongo database. and the objects where stored in that collections.
  
  ![](https://media.discordapp.net/attachments/352624122301513730/830993507586015262/unknown.png)
  
  these names are assigned by default and are equals to model classes,
   if you need to change these names you need to add the annotation documentto models like that:
...
8. Implement the queries of the previous step using *derived query methods* in your repository interface. Is it possible to implement all of them?
  ...
  Is not possible to implements the 3 query because the model must be change and the users should have a collection  of todos.
